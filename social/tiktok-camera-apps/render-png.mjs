...
const ORDER = ['Main', 'Blackmagic', 'Witness', 'Kino', 'FilmicPro', 'FinalCutCamera', 'Outro'];
const images = {};
for (const f of readdirSync('.').filter((f) => f.endsWith('.png') && !f.startsWith('slide-'))) {
  images[f] = 'data:image/png;base64,' + readFileSync(f).toString('base64');
}
