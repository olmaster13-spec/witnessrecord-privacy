...
App icons live in this folder as `icon-<stem>.png` and were supplied by hand: the
build box has no network egress, so nothing here can fetch them. The build uses any
`icon-<stem>.png` it finds and falls back to a dashed placeholder tile otherwise.
